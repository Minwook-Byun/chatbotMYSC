...
def get_rag_chain_for_guideline(_guideline_vector_store, _llm, guideline_name_for_prompt):
    ...
    11. fixed_accounting_guideline.txt에 대한 특수처리 
        ...
        - 중요한 사안으로, 사업비/지원금 계획을 검토할 때는 기업의 사용 방식을 한 번 이야기해주어야 합니다. 그리고 이어서 관련 지침을 이야기해주고 네가 판단한 근거를 이야기 해야 합니다. (예: 제미나이는 사업비 계획을 전문가 활용에 50% 이상 활용하고 있으며 금액은 0원입니다. 가이드 상 그러한 사용은 가능/불가능하며 이로 인해서 적합/부적합합니다.)
